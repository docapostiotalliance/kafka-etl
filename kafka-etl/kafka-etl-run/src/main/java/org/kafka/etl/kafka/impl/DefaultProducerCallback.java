...
public class DefaultProducerCallback implements IProducerCallback {
    ...
    public Callback producerCallback(String event, ProducerRecord<String, String> record) {
        return (metadata, e) -> {
            if (e != null) {
                LOGGER.error("[DefaultProducerCallback][producerCallback] Error while sending record in Kafka: {} - {} - {}", e.getMessage(), e.getClass().getSimpleName(), record.value(), e);
            } else {
                LOGGER.info("[DefaultProducerCallback][producerCallback] offset = {}, partition = {}, topic = {}", metadata.offset(), metadata.partition(), metadata.topic());
            }
        };
    }
}
